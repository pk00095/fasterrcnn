...
def get_resnet50_default_models(weights='imagenet', input_shape=(None, None, 3)):

    backbone = ResNet50(weights=weights, include_top=False, input_shape=input_shape)

    proposal_features = backbone.get_layer('conv4_block6_out').output
    proposal_classifier_features = backbone.get_layer('conv5_block3_out').output

    proposal_feature_extractor_model = keras.Model(inputs=backbone.inputs, outputs=proposal_features)

    box_classifier_model = keras.model(inputs=proposal_features, outputs=proposal_classifier_features)

    return proposal_feature_extractor_model, box_classifier_model
...
class FasterRCNNResnetKerasFeatureExtractor(
  ...
  def __init__(self,
    ...
    super(FasterRCNNResnetKerasFeatureExtractor, self).__init__(first_stage_features_stride, batch_norm_trainable,
        weight_decay)
    ...
    self.classification_backbone = ResNet50(
          weights='imagenet',
          include_top=False,
  ...
  def get_proposal_feature_extractor_model(self, name=None):
    ...
    proposal_features = self.classification_backbone.get_layer('conv4_block6_out').output
  ...
  def get_box_classifier_feature_extractor_model(self, name=None):
    ...
    """
    proposal_feature_maps = self.classification_backbone.get_layer('conv4_block6_out').output
    proposal_classifier_features = self.classification_backbone.get_layer(
            name='conv5_block3_out').output

    keras_model = tf.keras.model(inputs=proposal_feature_maps, outputs=proposal_classifier_features)
